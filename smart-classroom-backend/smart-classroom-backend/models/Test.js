/**
 * Test Model
 * Comprehensive test and exam management system
 */
...
const testSchema = new mongoose.Schema({
  ...
  instructions: {
    ...
    warningMessages: {
      timeWarning: {
        type: String,
        default: 'You have 5 minutes remaining'
      },
      navigationWarning: {
        type: String,
      ...
      submitWarning: {
        type: String,
        default: 'Are you sure you want to submit the test?'
      }
    }
  },
...
testSchema.methods.isEligible = async function(studentId) {
  ...
  // Additional eligibility checks would be implemented here
  return { eligible: true };
};

testSchema.methods.addQuestion = function(questionId, marks, orderIndex) {
  ...
  this.questions.push({
    questionId,
    marks,
    orderIndex: orderIndex || this.questions.length + 1
  });
...
testSchema.methods.publish = function() {
  ...
  if (this.schedule.startDate <= new Date()) {
    throw new Error('Cannot publish test with past start date');
  }
...
testSchema.statics.findByCourse = function(courseId) {
  return this.find({ courseId, status: { $in: ['published', 'active'] } });
};
